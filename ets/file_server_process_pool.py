# file_server_process_pool.py (Versi yang bekerja, dimodifikasi untuk statistik)
...
import os
import json
from concurrent.futures import ProcessPoolExecutor, as_completed # as_completed penting
...
# --- Variabel global di PROSES UTAMA untuk statistik ---
# Ini akan diupdate oleh proses utama berdasarkan hasil dari worker
# Tidak perlu multiprocessing.Value/Lock di sini karena hanya diakses oleh proses utama
server_worker_stats_main = {
    "processed_tasks": 0,
    "successful_tasks": 0,
    "failed_tasks": 0,
}
# Lock ini untuk mengamankan akses ke server_worker_stats_main JIKA kita mengupdatenya
# dari callback di thread yang berbeda di proses utama. Untuk pendekatan dengan as_completed,
# kita bisa update secara serial.
main_stats_lock = threading.Lock() # Jika Server.run() itu sendiri adalah thread

# (Konfigurasi logging utama di __main__ guard seperti sebelumnya)
from file_protocol import FileProtocol 

# Fungsi worker yang akan dijalankan oleh proses di pool
def process_client_connection(connection_socket, client_address):
    # Konfigurasi logging untuk proses worker ini
    ...
    command_buffer = ""
    connection_successful = True # Default sukses
    try:
        # ... (SELURUH LOGIKA INTI process_client_connection SAMA SEPERTI KODE ANDA YANG SUDAH BEKERJA) ...
        # Pastikan connection_successful diatur ke False jika ada error
        while True:
            ...
            if data:
                try:
                    ...
                    connection_successful = False # Tandai gagal
                    break
                ...
                if "\r\n\r\n" in command_buffer:
                    ...
                    try: # Cek status dari hasil_json_str yang valid
                        response_dict = json.loads(hasil_json_str)
                        if response_dict.get("status") == "ERROR":
                            connection_successful = False
                    except json.JSONDecodeError:
                        connection_successful = False # Gagal parse JSON berarti error
                ...
                break # Keluar loop jika client disconnect
        ...
        # Kembalikan status keberhasilan task ini
        return connection_successful


# ... (handle_error_response_worker sama) ...
...
class Server(threading.Thread):
    def __init__(self, ipaddress='0.0.0.0', port=6666, max_workers=2):
        ...
        self.running = True
        self.submitted_futures = [] # Untuk melacak future yang di-submit
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                self.submitted_futures.append(future) # Simpan future
        ...
        self.shutdown_pool_and_collect_stats() # Ganti nama fungsi

    def shutdown_pool_and_collect_stats(self): # Nama fungsi diubah
        global server_worker_stats_main, main_stats_lock # Akses statistik global utama
        self.main_logger.info("Processing results from worker tasks...")
        
        # Kumpulkan hasil dari future yang sudah selesai
        # Ini akan memblokir sampai semua future selesai jika wait=True di shutdown nanti
        # atau kita bisa proses yang sudah selesai dengan as_completed
        
        # Cara 1: Jika kita shutdown pool dulu lalu proses future (kurang ideal jika future banyak)
        # self.process_pool.shutdown(wait=True)
        # for future in self.submitted_futures:
        #     try:
        #         task_successful = future.result(timeout=1) # Ambil hasil, dengan timeout kecil
        ...
        #     except TimeoutError:
        #         self.main_logger.warning("Timeout getting result from a worker future during shutdown.")
        #         with main_stats_lock: # Hitung sebagai gagal jika timeout
        #             server_worker_stats_main["processed_tasks"] += 1
        #             server_worker_stats_main["failed_tasks"] += 1
        #     except Exception as e:
        #         self.main_logger.error(f"Error getting result from a worker future: {e}", exc_info=True)
        #         with main_stats_lock: # Hitung sebagai gagal
        #             server_worker_stats_main["processed_tasks"] += 1
        #             server_worker_stats_main["failed_tasks"] += 1
        
        # Cara 2: Gunakan as_completed SEBELUM shutdown pool jika ingin proses saat berjalan (lebih kompleks)
        # Untuk kesederhanaan saat shutdown, kita akan tunggu poolnya mati dulu.
        # Tapi ProcessPoolExecutor.shutdown() tidak mengembalikan futures.
        # Jadi, kita harus pastikan semua task selesai.

        self.main_logger.info("Shutting down process pool... (waiting for tasks to complete)")
        self.process_pool.shutdown(wait=True) # Pastikan semua task selesai
        self.main_logger.info("Process pool shut down.")

        # Setelah pool shutdown, kita bisa iterasi futures yang kita simpan
        for future in self.submitted_futures:
            if future.done(): # Seharusnya semua sudah done karena wait=True
                try:
                    task_successful = future.result(timeout=0.1) # Timeout sangat kecil karena harusnya sudah selesai
                    with main_stats_lock:
                        server_worker_stats_main["processed_tasks"] += 1
                        if task_successful:
                            server_worker_stats_main["successful_tasks"] += 1
                            ...
                            server_worker_stats_main["failed_tasks"] += 1
                except Exception as e: # Exception saat mengambil result()
                    self.main_logger.error(f"Error retrieving result from completed future: {e}")
                    with main_stats_lock:
                        server_worker_stats_main["processed_tasks"] += 1
                        server_worker_stats_main["failed_tasks"] += 1
            else:
                # Ini seharusnya tidak terjadi jika shutdown(wait=True) bekerja
                self.main_logger.warning("Found a future that was not done after pool shutdown!")
                with main_stats_lock:
                    server_worker_stats_main["processed_tasks"] += 1
                    server_worker_stats_main["failed_tasks"] += 1 # Hitung sebagai gagal

        # Tampilkan statistik
        self.main_logger.info("=" * 30 + " SERVER WORKER STATISTICS (PROCESS POOL) " + "=" * 30)
        with main_stats_lock: # Amankan pembacaan juga
...
# ... (main dan if __name__ == "__main__" guard sama seperti sebelumnya) ...
def main():
    ...
    try:
        ...
        if svr.is_alive():
            svr.stop_server() 
            svr.join(timeout=15) # Beri waktu lebih untuk pool shutdown dan kumpul stats
            if svr.is_alive():
